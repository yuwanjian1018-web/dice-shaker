...
`cup-material-v3.blend`、`cup-material-v3.glb` 为用户确认的完整模型；骰子沿用此前的象牙色圆角造型及 21 个哑光球面凹点。2 点面上的两个凹点直径单独放大 12%，对应的本体凹槽同步放大，其余五个点数面不变。`mobile-assets.json` 记录原模型哈希与移动资源规格。
...
- 合盖摇动只绘制 6,720 个三角形；完全打开五颗时绘制 104,700 个三角形。摇动最后 260 ms 平滑收小位移和倾角，归零帧、新骰子合批和静态高分辨率恢复分帧执行；静止时不循环绘制，后台暂停，离开页面释放 GPU 资源。
